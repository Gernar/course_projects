...
def FakeClusterUpCorrect(average_distance_array, core_list, cluster_list, lines):
	...
	if max_dist > 2*aver_aver_dist or min_dist == max_dist:
		...
		new_core = lines[a.index(max_dist_ind)] if all(lines[a.index(max_dist_ind)] != core_list[max_dist_ind]) else lines[len(a) - a[-1::-1].index(max_dist_ind) - 1]
...
def ClusterDownCorrect(core_list, lines, cluster_list, amount_of_vectors, optimal_average_distance):
	...
	for i, core1 in enumerate(core_list[:-1]):
		for j, core2 in enumerate(core_list[i+1:]):
			near_pairs = SetNearestVectors(from_core = i, to_core = j+i+1, core_list = core_list, cluster_list = cluster_list, lines = lines, amount_of_vectors = amount_of_vectors)
			...
			if aver_dist < optimal_average_distance:
				...
				new_core_list, new_cluster_list = ClusterDownCorrect(core_list = new_core_list, lines = lines, cluster_list = new_cluster_list, 
					amount_of_vectors = amount_of_vectors, optimal_average_distance = optimal_average_distance)
				flag = True
				break
		if flag:
				break	
...
def ClusterCorrect(core_list, lines, cluster_list, amount_of_vectors, optimal_average_distance):
	...
	#print core_list, new_core_list
	new_core_list, new_cluster_list = Clustering(old_core_list = new_core_list, cluster_list = new_cluster_list, lines = lines)
	new_core_list, new_cluster_list = ClusterDownCorrect(core_list = new_core_list, lines = lines, cluster_list = new_cluster_list,
		amount_of_vectors = amount_of_vectors, optimal_average_distance = optimal_average_distance)
...
def SortCores(core_list, cluster_list, lines, amount_of_vectors):
	...
	near_pairs =  SetNearestVectors(from_core = 0, to_core = 1, core_list = core_list, cluster_list = cluster_list, lines = lines, amount_of_vectors=amount_of_vectors)
	min_dist = AverageDistOfNearestVecs(near_pairs)

	for i, core1 in enumerate(core_list[:-1]):
		for j, core2 in enumerate(core_list[i+1:]):
			near_pairs =  SetNearestVectors(i, i+j+1, core_list = core_list, cluster_list = cluster_list, lines = lines, amount_of_vectors=amount_of_vectors)
			dist = AverageDistOfNearestVecs(near_pairs)
			if dist < min_dist:
				min_dist = dist
				first_index_core_pair = [i, j+i+1]
	#print first_index_core_pair
	...
	for i, core1 in enumerate(core_list[1:-1]):
		min_dist_pairs =  SetNearestVectors(i, i+1, core_list = core_list, cluster_list = cluster_list, lines = lines, amount_of_vectors=amount_of_vectors)
		min_dist = AverageDistOfNearestVecs(min_dist_pairs)
		min_dist_index = i+1
		for j, core2 in enumerate(core_list[i+1:]):
			near_pairs =  SetNearestVectors(i, j+i+1, core_list = core_list, cluster_list = cluster_list, lines = lines, amount_of_vectors=amount_of_vectors)
			dist = AverageDistOfNearestVecs(near_pairs)
			if dist < min_dist:
				min_dist = dist
				min_dist_index = j+i+1
		#print i+1, min_dist_index
